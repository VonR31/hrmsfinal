...
// Initialize the attendance records array
$attendanceRecords = array();

// Fetch all employeeIds of employees
$selectEmployeeIdsQuery = "
    SELECT 
        employeeId
    FROM 
        user
...
$stmt = $conn->prepare($selectEmployeeIdsQuery);
$stmt->execute();
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    if ($result_attendance->num_rows > 0) {
      while ($row_attendance = $result_attendance->fetch_assoc()) {
        $attendanceRecords[] = $row_attendance;
      }
    }
