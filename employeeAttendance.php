...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($checkInTime) {
        ...
        $existingRecordQuery = "SELECT * FROM attendance WHERE employeeId = ? AND date = ?";
        $stmt = $conn->prepare($existingRecordQuery);
        $stmt->bind_param("is", $employeeId, $date);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows > 0) {
            // Record already exists, update it with check-in time
            $updateQuery = "UPDATE attendance SET checkInTime = ?, status = ? WHERE employeeId = ? AND date = ?";
            $stmt = $conn->prepare($updateQuery);
            $stmt->bind_param("ssis", $checkInTime, $status, $employeeId, $date);
        } else {
            // Insert a new record for check-in
            $insertQuery = "INSERT INTO attendance (userId, employeeId, date, status, checkInTime) VALUES (?,?,?,?,?)";
            $stmt = $conn->prepare($insertQuery);
            $stmt->bind_param("iisss", $userId, $employeeId, $date, $status, $checkInTime);
        }
    } elseif ($checkOutTime) {
        // Update the existing record for check-out
        $updateQuery = "UPDATE attendance SET checkOutTime = ?, status = ? WHERE employeeId = ? AND date = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("ssis", $checkOutTime, $status, $employeeId, $date);
    } elseif ($status) {
        // Insert a new record for absent or leave
        $insertQuery = "INSERT INTO attendance (employeeId, date, status) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($insertQuery);
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ml-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="employeePayroll.php">Payroll</a>
                    </li>
                    <li class="nav-item">
