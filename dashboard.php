...
$sqlLeave = "SELECT COUNT(*) as leaveCount FROM employee_leave WHERE status = 'Pending'";
...
$sqlAttendance = "SELECT d.departmentName as department, a.status, COUNT(*) as count 
                  FROM attendance a
...
if ($resultAttendance->num_rows > 0) {
  while ($rowAttendance = $resultAttendance->fetch_assoc()) {
    $department = $rowAttendance['department'];
    $status = $rowAttendance['status'];
    $count = $rowAttendance['count'];

    if (!isset($attendanceData[$department])) {
      $attendanceData[$department] = ['Present' => 0, 'Absent' => 0, 'On Going Leave' => 0];
    }
    $attendanceData[$department][$status] = $count;
  }
}
...
<body>
      ...
      <div class="sidebar-inner slimscroll">
        <div class="sidebar-contents">
          <div id="sidebar-menu" class="sidebar-menu">
            ...
            <ul class="logout">
              <li>
                <a href="logout.php"><img src="assets/img/logout.svg" alt="sidebar_img"><span> Logout</span></a>
              </li>
            </ul>
          </div>
        </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      var departments = Object.keys(attendanceData);
      var seriesData = ['Present', 'Absent', 'On Going Leave'].map(status => {
        return {
          name: status,
          data: departments.map(department => attendanceData[department][status])
        };
      });
      ...
      var attendanceOptions = {
        ...
        colors: ['#4CAF50', '#FF5252', '#FFC107'] // Green for Present, Red for Absent, Yellow for On Leave
      };
